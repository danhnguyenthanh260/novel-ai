...
import { getWorkflowContextDigest, type WorkflowCommandScope } from "@/features/scenes/server/workflow/statusService";
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ slug: string }> }): Promise<NextResponse> {
  ...
  try {
    const item = await getWorkflowContextDigest(pool, {
      storySlug: slug,
    ...
    const message = error instanceof Error ? error.message : "WORKFLOW_CONTEXT_FAILED";
