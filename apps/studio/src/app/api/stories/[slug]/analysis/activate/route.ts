...
import { activateHistorianSnapshot } from "@/features/analysis/server/historianAnalysisService";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const activatedBy = typeof body.activated_by === "string" ? body.activated_by : "operator";
    const result = await activateHistorianSnapshot(storyId, {
    ...
    const msg = error instanceof Error ? error.message : "ANALYSIS_ACTIVATE_FAILED";
