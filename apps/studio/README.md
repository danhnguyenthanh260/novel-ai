...
- Chat-first command handlers must follow the secondary workspace link policy in [chat-first-architecture-policy.md](../../docs/operations/implementation/chat-first-architecture-policy.md). Command handlers may add secondary workspace links only after the core chat result, timeline block, or inspector payload has rendered.
...
- Brainstorm follow-up actions such as scene goal, character contradiction, and chapter opening stay in chat unless the user explicitly asks to write, draft, generate, or run AutoWrite for a chapter.
