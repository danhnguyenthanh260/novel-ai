...
| `NARRATIVE_CRITIC` | `worker_narrative_handlers.py` | `process_narrative_critic_task` | prose and rubric/context | critique result; may enqueue refine | `call_llm_json` | compatibility-only | Useful behavior has a V3 home as internal `CHAPTER_WRITE_V3_CRITIC`; public task dispatch remains only for legacy compatibility until retirement. |
| `NARRATIVE_REFINE` | `worker_narrative_handlers.py` | `process_narrative_refine_task` | prose + critique | refined prose; enqueues next/finalize | `call_llm_text` | compatibility-only | Useful behavior has a V3 home as one-pass internal `CHAPTER_WRITE_V3_REFINE`; public task dispatch remains only for legacy compatibility until retirement. |
...
- Useful `NARRATIVE_CRITIC/REFINE` behavior is merged into Python `CHAPTER_WRITE_V3` internals; remaining `NARRATIVE_STYLIST` behavior is `Investigate` until public dispatch retirement.
